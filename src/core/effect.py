...
def fire_n_smoke_3 (parent, store=None,
    ...
    fire2 = PolyExhaust(
        parent=parent,
        pos=Point3(fx_uniform(0, parent._size_xy * 0.8), fx_uniform(0, parent._size_xy * 0.8), 0.0),
        radius0=1.0 * sclfact,
        radius1=3.0 * sclfact,
    ...
    if spos1 is not None:
        # smoke1 = PolyBraid(
            ...
            # # # # thickness=7.0 * sclfact,
            # # # # endthickness=19.0 * sclfact,
            # spacing=1.0,
            # partvel=Vec3(0, 0, 40 * forcefact),
            # emittang=Vec3(0, 0, 1),
            # emitnorm=Vec3(1, 0, 0),
            # offang=None,
            # offrad=[0.4 * emradfact * parent._size_xy, 0.2 * emradfact * parent._size_xy , 0.3 * emradfact * parent._size_xy],
            # # # # offrad=0.3 * emradfact * parent._size_xy,
            # offtang=[0.0, 0.33, 0.66],
            # # # # offtang=0.0,
            # randang=True,
            # randrad=True,
        ...
        smoke1 = PolyExhaust(
            ...
            subnode=None,
            pdir=Vec3(0,0,1),
            emradius=0.4 * emradfact * parent._size_xy,
            texture="images/particles/smoke6-1.png",
            glowmap=pycv(py=rgba(255, 255, 255, 1.0), c=rgba(0, 0, 0, 0.1)),
